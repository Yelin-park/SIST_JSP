...
<%
	Member member = new Member("admin", "관리자", 20, "서울시", "010-1234-5678");
	session.setAttribute("user1", member);
%>

<h3>세션에서 member 객체인 user를 얻어와서 출력</h3>
<%
	Member user = (Member)session.getAttribute("user1");
%>
id = <%= user.getId() %><br>
name = <%= user.getName() %><br>

<!-- EL 안에서 세션에 있는 user 객체를 바로 사용할 수 있다. -->
<!-- session.get에서 가져와 담아놓은 Member 객체 변수 user로 가져오는 것 -->
age = ${user.age }<br>
addr = ${user.addr }<br>
tel = ${user.tel }<br>
</body>
